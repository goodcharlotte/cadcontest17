...
void Io_WriteVerilogObjects( FILE * pFile, Abc_Ntk_t * pNtk )
{
    ...
    if ( Abc_NtkHasMapping(pNtk) )
    {
        ...
        Abc_NtkForEachNode( pNtk, pObj, k )
        {
            ...
            {
                //fprintf( pFile, "  %-*s ", Length, Mio_GateReadName(pGate));
                //fprintf( pFile, "%d ",  strlen(Mio_GateReadName(pGate)) );
                char *gname = Mio_GateReadName(pGate);
                size_t sz;
                fprintf( pFile, "  ");
                for (sz = 0; sz < strlen(Mio_GateReadName(pGate)); sz++ ) {
                    if (!(gname[sz] >= 48 && gname[sz] <= 57)) fprintf( pFile, "%c", gname[sz]);
                }
                fprintf( pFile, "  ");
